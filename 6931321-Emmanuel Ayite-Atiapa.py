# -*- coding: utf-8 -*-
"""
Created on Fri Feb 17 09:06:24 2023
...
V=w*(L/2-x)
print( )
print('(e) The shea force for each step alongthe span is {}'.format(V))
...
#Question h
"""Let th maximum bending moment be max_M and the minimum bending moment be min_M"""
#for the maximum
max_M= max(M)
"""When the bending moment is max_M,we get an expression x**2-Lx +(L**2/6)+(2*max_M)/w=0 """
a=1
b=-L
c=(L**2/6)+(2*max_M)/w  #Using the Almighty formula the two roots are;
discriminant=b**2-4*a*c
root_1=(-b+np.sqrt(discriminant))/2*a
root_2=(-b-np.sqrt(discriminant))/2*a
print( )
print('(h.1) The points at which the maximum bending moment occurs are {0} and {1}'.format(root_1,root_2))
#for the minimum 
min_M=min(M)
...
root_2=(-b+np.sqrt(discriminant))/2*a
print( )
print('(h.2)The points at which the minimumbending moment occurs are {0} and {1}'.format(root_1,root_2))